...
async function ensureProjectDriveFolders(tenant, meta) {
  if (meta.aportesFolderId && meta.reportesFolderId) {
    return { aportesFolderId: meta.aportesFolderId, reportesFolderId: meta.reportesFolderId };
  }
  const driveConfig = await getDriveConfig(tenant);
  if (!driveConfig?.folderId) return null;

  const projectFolderId = await ensureSubfolder(driveConfig.folderId, meta.name);
  const aportesFolderId = await ensureSubfolder(projectFolderId, 'Aportes');
  const reportesFolderId = await ensureSubfolder(projectFolderId, 'Reportes');

  const current = (await kv.get(experimentMetaKey(tenant, meta.id))) ?? meta;
  await kv.set(experimentMetaKey(tenant, meta.id), { ...current, driveFolderId: projectFolderId, aportesFolderId, reportesFolderId });
  return { aportesFolderId, reportesFolderId };
}
    ...
    try {
      ...
      if (drive) {
        ...
        const uploaded = await Promise.all(
          cleanEvidence.map((att, i) => uploadFile(testFolderId, {
            name: `${datePrefix}_${contributor}_${(att.name || `evidencia-${i + 1}`).trim()}`,
            mimeType: att.mimeType,
            data: att.data,
          }))
        );
        execution.evidence = cleanEvidence.map((att, i) => ({ ...att, driveFileId: uploaded[i].id, driveUrl: uploaded[i].webViewLink }));
      }
    } catch (err) {
