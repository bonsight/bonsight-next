...
export async function createExperiment(tenant, {
  ...
  const id = generateId();
  const now = Date.now();
  // projectKind es el interruptor de COMPORTAMIENTO (experimental vs civil — qué pestañas
  // tiene el proyecto) — no confundir con `type`, que sigue siendo la etiqueta libre
  // ("Estructural", "Civil", etc.) que ya existía antes y el Director edita en Detalles.
  const kind = projectKind === 'civil' ? 'civil' : 'experimental';

  const { linkedTasks, linkedPartidas } = kind === 'civil'
    ? linkTasksToPartidas(tasks, partidas)
    : { linkedTasks: [], linkedPartidas: [] };

  let fechaInicioFinal = kind === 'civil' ? (fechaInicioProyecto || null) : null;
  let fechaFinFinal = kind === 'civil' ? (fechaFinProyecto || null) : null;
  if (kind === 'civil' && (!fechaInicioFinal || !fechaFinFinal) && linkedTasks.length) {
...
export async function updateTask(tenant, experimentId, taskId, patch) {
  ...
  for (const field of ['fase', 'nombre', 'responsable', 'fechaInicio', 'fechaFin', 'partidaId']) {
...
export async function setTaskProgreso(tenant, experimentId, taskId, progreso, by) {
  ...
  tasks[idx] = { ...tasks[idx], progreso: done ? 100 : 0, updatedAt: new Date().toISOString() };
  await kv.set(tasksKey(tenant, experimentId), tasks);
  await addEvent(tenant, experimentId, {
    type: 'tarea',
    actor: by,
