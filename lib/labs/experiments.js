...
export async function summarizeExperiment(tenant, meta) {
  ...
  if (TASK_TRACKING_KINDS.includes(meta.projectKind)) {
    const [tasks, partidas] = await Promise.all([
    ...
    for (const t of tasksList) if (t.responsable) teamIds.add(t.responsable);
  } else {
...
export async function updateProjectDetails(tenant, id, { code, type, hasBudget, budgetAmount, budgetCurrency, status }, changedBy) {
  const meta = await kv.get(experimentMetaKey(tenant, id));
  if (!meta) throw new Error('Experimento no encontrado.');
  ...
  const defaults = { code: '', type: '', hasBudget: false, budgetAmount: null, budgetCurrency: '', status: 'activo' };
  const changes = [];
  const next = { ...meta };
