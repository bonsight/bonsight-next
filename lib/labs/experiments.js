...
const gastosKey = (tenant, id) => `labs:${tenant}:experiment:${id}:gastos`;
...
// Tareas viejas guardaban un solo `responsable` (string|null); ahora se guarda `responsables`
// (array, puede tener 0, 1 o más ids) — normaliza a array en cualquier punto de lectura para
// no tener que migrar los datos ya guardados en Redis.
export function taskResponsables(task) {
  if (Array.isArray(task?.responsables)) return task.responsables;
  return task?.responsable ? [task.responsable] : [];
}

const CRITERIA_OPERATOR_SYMBOLS = { '>': '>', '<': '<', '=': '=', '>=': '≥', '<=': '≤' };

// Un criterio de éxito es { label, operator, value, unit } desde que se volvió estructurado
// (antes era texto libre) — esto lo devuelve a texto legible para los prompts de IA. Soporta
// el string suelto también, por proyectos creados antes de este cambio.
export function formatSuccessCriterion(c) {
  if (typeof c === 'string') return c;
  const op = CRITERIA_OPERATOR_SYMBOLS[c.operator] ?? c.operator ?? '';
...
// directo en la partida (ejecutado ahora se deriva SIEMPRE de sus gastos, ver
// applyGastosToPartidas), se crea un gasto sintético marcado `imported: true` para no perder
// el dato, dejando claro en la UI que ese monto no tiene comprobante adjunto.
function linkTasksToPartidas(rawTasks, rawPartidas) {
  ...
  const partidas = (Array.isArray(rawPartidas) ? rawPartidas : []).map((p) => {
    ...
    return {
      ...
      origen: 'inicial', // planificada al crear el proyecto (Excel), a diferencia de "+ Nueva partida" después
      createdAt: now,
      updatedAt: now,
    };
  });
  ...
  const tasks = (Array.isArray(rawTasks) ? rawTasks : []).map((t) => ({
    ...
    fase: t.fase || '',
    nombre: t.nombre || '',
    responsables: Array.isArray(t.responsables) ? t.responsables : (t.responsable ? [t.responsable] : []),
  ...
  const withVisibility = await Promise.all(
    all.map(async (m) => {
      if (TASK_TRACKING_KINDS.includes(m.projectKind)) {
        ...
        return tasks.some((t) => taskResponsables(t).includes(user.id)) ? m : null;
      }
...
// "Ejecutado" ya no se guarda en la partida — se deriva sumando sus gastos (cada uno con
// factura de respaldo opcional) cada vez que se leen las partidas. Así computeCivilMetrics /
// computeCivilReportBreakdown / computeCivilAlerts (que solo leen p.ejecutado) no necesitan
// cambiar — reciben partidas ya con el total correcto.
function applyGastosToPartidas(partidas, gastos) {
  const sums = new Map();
  for (const g of gastos) {
    sums.set(g.partidaId, (sums.get(g.partidaId) || 0) + (g.monto || 0));
  }
  return partidas.map((p) => ({ ...p, ejecutado: sums.get(p.id) || 0 }));
}

// Datos livianos para las tarjetas del picker — avance, presupuesto (si aplica) y equipo
// (Supervisores asignados + Registradores con al menos una tarea/prueba en el proyecto).
// Una consulta extra por proyecto (tasks o tests), aceptable para la cantidad de proyectos
// por tenant que maneja Labs hoy — si esto crece mucho, se puede cachear en meta.
export async function summarizeExperiment(tenant, meta) {
  const teamIds = new Set(meta.supervisorIds || []);
  let progressPct = null;
  let presupuesto = null;
  let ejecutado = null;

  if (TASK_TRACKING_KINDS.includes(meta.projectKind)) {
    const [tasks, partidas, gastos] = await Promise.all([
      kv.get(tasksKey(tenant, meta.id)),
      kv.get(partidasKey(tenant, meta.id)),
      kv.get(gastosKey(tenant, meta.id)),
    ]);
    const tasksList = tasks ?? [];
    const partidasList = applyGastosToPartidas(partidas ?? [], gastos ?? []);
    const m = computeCivilMetrics(tasksList, partidasList, meta);
    progressPct = m.totalTareas ? m.pctTareas : null;
    if (meta.projectKind === 'civil' && m.totalImporte > 0) {
      presupuesto = m.totalImporte;
      ejecutado = m.totalEjecutado;
    }
    for (const t of tasksList) for (const rid of taskResponsables(t)) teamIds.add(rid);
  } else {
    const tests = (await kv.get(testsKey(tenant, meta.id))) ?? [];
    for (const t of tests) (t.registradorIds || []).forEach((id) => teamIds.add(id));
  }

  return { ...meta, progressPct, presupuesto, ejecutado, teamIds: [...teamIds] };
}

export async function getExperiment(tenant, id) {
  const [meta, tests, executions, events, feedback, reports, documents, tasks, partidas, gastos] = await Promise.all([
  ...
  const partidasList = applyGastosToPartidas(partidas ?? [], gastosList);
...
export const EXPERIMENT_STATUSES = ['activo', 'pausado', 'completado'];

// Pausar/completar un proyecto tiene que bloquear la gestión de verdad, no ser solo una
// etiqueta — se usa en las routes de tareas/partidas antes de dejar crear/editar/eliminar.
...
// Código/tipo/presupuesto/estado — a diferencia de supervisorIds, estos campos quedan con
// historial de cambios (quién, cuándo, qué cambió), consultable solo por el Director (ver
// route). Pausar/completar un proyecto pasa por acá, no por un endpoint aparte, para que
// quede en el mismo historial que el resto de los cambios de detalle.
export async function updateProjectDetails(tenant, id, { code, type, hasBudget, budgetAmount, budgetCurrency, status, purpose, hypothesis, successCriteria }, changedBy) {
  const meta = await kv.get(experimentMetaKey(tenant, id));
  if (!meta) throw new Error('Experimento no encontrado.');
  ...
  if (status !== undefined && EXPERIMENT_STATUSES.includes(status)) nextValues.status = status;
  // Solo proyectos experimental — código/tipo/presupuesto son de civil/seguimiento, esto es
  // lo que se llena al crear un proyecto experimental (ver CreateExperimentModal).
  if (purpose !== undefined) nextValues.purpose = purpose ?? '';
  if (hypothesis !== undefined) nextValues.hypothesis = hypothesis ?? '';

  const defaults = { code: '', type: '', hasBudget: false, budgetAmount: null, budgetCurrency: '', status: 'activo', purpose: '', hypothesis: '' };
  const changes = [];
  const next = { ...meta };
  ...
  // successCriteria es un array — se compara como texto formateado (no por referencia) para
  // saber si realmente cambió, y así queda legible en el historial en vez de [object Object].
  if (successCriteria !== undefined) {
    const cleanCriteria = Array.isArray(successCriteria) ? successCriteria : [];
    const oldFormatted = (meta.successCriteria || []).map(formatSuccessCriterion).join(' · ') || '(ninguno)';
    const newFormatted = cleanCriteria.map(formatSuccessCriterion).join(' · ') || '(ninguno)';
    if (oldFormatted !== newFormatted) {
      changes.push({ field: 'successCriteria', from: oldFormatted, to: newFormatted });
      next.successCriteria = cleanCriteria;
    }
  }
...
async function ensureProjectDriveFolders(tenant, meta) {
  if (meta.aportesFolderId && meta.reportesFolderId && meta.documentosFolderId && meta.comentariosFolderId && meta.gastosFolderId) {
    return { aportesFolderId: meta.aportesFolderId, reportesFolderId: meta.reportesFolderId, documentosFolderId: meta.documentosFolderId, comentariosFolderId: meta.comentariosFolderId, gastosFolderId: meta.gastosFolderId };
  }
  ...
  // Aportes/Reportes de antes de que existiera Documentación (o Comentarios, o Gastos), sin
  // recrear lo que ya existe.
  ...
  const gastosFolderId = meta.gastosFolderId || await ensureSubfolder(projectFolderId, 'Gastos');

  const current = (await kv.get(experimentMetaKey(tenant, meta.id))) ?? meta;
  await kv.set(experimentMetaKey(tenant, meta.id), { ...current, driveFolderId: projectFolderId, aportesFolderId, reportesFolderId, documentosFolderId, comentariosFolderId, gastosFolderId });
  return { aportesFolderId, reportesFolderId, documentosFolderId, comentariosFolderId, gastosFolderId };
}
...
export async function createTest(tenant, experimentId, { name, icon, fields, registradorIds }) {
  ...
  const test = {
    ...
    name: name.trim(),
    icon: icon || '🧪',
    // operator/value/unit son opcionales — un campo de texto (ej. "Proveedor") no tiene
    // criterio de paso/falla, uno numérico (ej. "Humedad") puede tenerlo, mismo formato que
    // los Criterios de éxito del proyecto (ver CreateExperimentModal).
    fields: fields.map((f) => {
...
export async function addTask(tenant, experimentId, { fase, nombre, responsables, fechaInicio, fechaFin, duracionDias, partidaId }) {
...
export async function updateTask(tenant, experimentId, taskId, patch) {
  ...
  for (const field of ['fase', 'nombre', 'responsables', 'fechaInicio', 'fechaFin', 'partidaId', 'status']) {
...
export async function addPartida(tenant, experimentId, { etapa, descripcion, cantidad, unidad, precioUnitario, proveedor, comentarios }) {
  ...
  const partida = {
    ...
    origen: 'agregada', // agregada después de creado el proyecto, vía "+ Nueva partida"
    createdAt: now,
    updatedAt: now,
  };
...
// `ejecutado` NO es un campo editable acá — se deriva siempre de los gastos (ver addGasto /
// applyGastosToPartidas). Si patch trae `ejecutado`, se ignora a propósito: esta función ya
// no puede pisar el total con un número suelto sin factura de respaldo.
...
// ── Gastos (cada uno con su factura de respaldo) — lo que ejecuta una partida ──────────────
export async function getGastosList(tenant, experimentId) {
  return (await kv.get(gastosKey(tenant, experimentId))) ?? [];
}

// La factura queda como respaldo del gasto — mismo mecanismo best-effort que addFeedback (si
// el tenant no tiene Drive conectado o la subida falla, el adjunto queda igual en Redis con
// su `data` base64, nunca bloquea el guardado del gasto).
export async function addGasto(tenant, experimentId, { partidaId, monto, fecha, proveedor, nota, attachments, createdBy }) {
  ...
  const [partidas, gastos, meta] = await Promise.all([
    getPartidasList(tenant, experimentId),
    getGastosList(tenant, experimentId),
    kv.get(experimentMetaKey(tenant, experimentId)),
  ]);
  ...
  if (pending.length && meta) {
    try {
      ...
      if (drive) {
        const datePrefix = entry.createdAt.slice(0, 10);
        const uploaded = await Promise.all(
          pending.map((att, i) => uploadFile(drive.gastosFolderId, {
            name: `${datePrefix}_${partida.descripcion}_${createdBy}_${(att.name || `factura-${i + 1}`).trim()}`,
            mimeType: att.mimeType,
            data: att.data,
          }))
        );
      ...
      console.error(`[labs] no se pudo subir factura de gasto a Drive (${tenant}/${experimentId}):`, err.message);
    }
  }

  await kv.set(gastosKey(tenant, experimentId), [entry, ...gastos]);
  await addEvent(tenant, experimentId, {
    type: 'gasto',
    actor: createdBy,
...
// Sin edición a propósito — si el monto está mal, se borra y se agrega de nuevo, así el gasto
// que queda siempre corresponde exactamente a su factura adjunta.
export async function deleteGasto(tenant, experimentId, gastoId) {
  const gastos = await getGastosList(tenant, experimentId);
  await kv.set(gastosKey(tenant, experimentId), gastos.filter((g) => g.id !== gastoId));
  await touchExperiment(tenant, experimentId);
}
...
export function computeCivilMetrics(tasks, partidas, meta) {
  ...
  // Desglose planificado (partidas del plan inicial, ej. Excel) vs agregado después vía
  // "+ Nueva partida" — para ver cuánto se salió del presupuesto original, no solo el total.
