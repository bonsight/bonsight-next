...
import { syncKnownParticipants } from './participants';
...
async function buildParticipantMap(tenant) {
  ...
  const conversationsWithParticipants = await Promise.all(
    allConversations.map(async (conv) => {
      // Prefer confirmed participant (set via recognition flow) over message extraction
      if (conv.confirmedParticipantName) {
        return { ...conv, participant: conv.confirmedParticipantName, role: conv.confirmedParticipantRole ?? null };
      }
  ...
  // Group by first name (case-insensitive) — deduplicates "Harry", "Harry jose", "Harry Yupanqui"
  // into the most complete (longest) name. Confirmed names always win over extracted ones.
  const byFirstName = {};
  for (const conv of conversationsWithParticipants) {
    if (!conv.participant) {
      const uk = '__unknown__';
      if (!byFirstName[uk]) byFirstName[uk] = { name: null, role: null, convIds: [] };
      byFirstName[uk].convIds.push(conv.id);
      continue;
    }
    const firstKey = conv.participant.split(/\s+/)[0].toLowerCase();
    const existing = byFirstName[firstKey];
    if (!existing) {
      byFirstName[firstKey] = { name: conv.participant, role: conv.role ?? null, convIds: [conv.id], confirmed: !!conv.confirmedParticipantName };
    } else {
      // Confirmed name always wins; otherwise keep the longest
      const incomingConfirmed = !!conv.confirmedParticipantName;
      if (incomingConfirmed && !existing.confirmed) {
        existing.name = conv.participant;
        existing.role = conv.role ?? existing.role;
        existing.confirmed = true;
      } else if (!existing.confirmed && conv.participant.length > (existing.name?.length ?? 0)) {
        existing.name = conv.participant;
        existing.role = conv.role ?? existing.role;
      } else if (!existing.role && conv.role) {
        existing.role = conv.role;
      }
      existing.convIds.push(conv.id);
    }
  }
  const groups = byFirstName;
...
async function generateInsights(learnings, tenant) {
...
${top4.map((l, i) => `${i + 1}. ${l.content}`).join('\n')}`,
    ...
    trackUsage({ tenant, product: 'kai', feature: 'insights', model: 'claude-haiku-4-5-20251001', inputTokens: response.usage.input_tokens, outputTokens: response.usage.output_tokens }).catch(() => null);
...
async function generateTransversals(tenant, tenantName, participantContext) {
  try {
    const response = await anthropic.messages.create({
      model:      'claude-haiku-4-5-20251001',
      max_tokens: 800,
...
${participantContext}`,
    ...
    trackUsage({ tenant, product: 'kai', feature: 'transversals', model: 'claude-haiku-4-5-20251001', inputTokens: response.usage.input_tokens, outputTokens: response.usage.output_tokens }).catch(() => null);
  ...
  const [, transversals] = await Promise.all([
    ...
    Promise.all(
      participants.map(async (p) => {
        ...
        p.top_learnings  = await generateInsights(pLearnings, tenant);
      })
    ).then(async () => {
      const result = {
        participants,
    ...
    (async () => {
      ...
      const result      = await generateTransversals(tenant, tenantName, participantContext);
      await kv.set(transversalLearningsKey(tenant), result);
      return result;
    })(),
  ]);

  // Keep known_participants store in sync so recognition flow can match them
  syncKnownParticipants(tenant, participants).catch(() => null);
