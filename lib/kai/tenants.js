...
export async function updateTenantMeta(slug, updates) {
  const current = await getTenantMeta(slug);
  if (!current) throw new Error(`Tenant '${slug}' no encontrado`);
  await kv.set(metaKey(slug), { ...current, ...updates });
}
