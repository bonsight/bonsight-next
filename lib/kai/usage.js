...
export const FEATURE_LABELS = {
  ...
  supervisor_summary:             'Resumen Supervisor',
  director_brief:                 'Brief Director',
  contribution_interpret:         'Interpretar Aporte',
  report_draft:                   'Borrador de Reporte',
  civil_report_draft:             'Borrador de Reporte (Civil/Seguimiento)',
  feedback_conversion_suggestion: 'Sugerencia de Conversión',
};
...
export async function getTenantMonthlyUsage(tenant, month) {
  ...
  const total = PRODUCTS.reduce((acc, p) => ({
    cost:          acc.cost          + byProduct[p].cost,
    input_tokens:  acc.input_tokens  + byProduct[p].input_tokens,
    output_tokens: acc.output_tokens + byProduct[p].output_tokens,
    calls:         acc.calls         + byProduct[p].calls,
...
export async function getAllTenantsMonthlyUsage(tenants, month) {
  ...
  return tenants.map((t) => {
    const totals = { cost: 0, input_tokens: 0, output_tokens: 0, calls: 0 };
    for (const p of PRODUCTS) {
      const h = parseHash(results[idx++]);
      totals.cost          += h.cost;
      totals.input_tokens  += h.input_tokens;
      totals.output_tokens += h.output_tokens;
      totals.calls         += h.calls;
    }
    return { slug: t.slug, name: t.name, ...totals };
