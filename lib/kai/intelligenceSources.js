...
export const SOURCE_DEFINITIONS = [
  ...
  },
  {
    ...
    fields: [
      { key: 'integrationToken', label: 'Integration Token', placeholder: 'secret_...', type: 'password' },
    ],
    comingSoon: false,
  },
  {
...
export function buildSourcesContext(sources) {
  ...
  const level2Connected = active.filter((s) => s.evidenceLevel === 2 && !s.alwaysActive);
  ...
  if (level2Connected.length) {
    lines.push('Nivel 2 — Documentos y conocimiento conectado:');
    for (const s of level2Connected) {
      const extra = s.id === 'notion' && s.config?.databaseCount != null
        ? ` (${s.config.databaseCount} bases de datos, ${s.config.pageCount} páginas)`
        : '';
      lines.push(`  ✅ ${s.label}${extra} — ${s.capabilities.slice(0, 3).join(', ')}`);
    }
    lines.push('');
  }
