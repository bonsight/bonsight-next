...
Si la reunión repasa o define un flujo de trabajo con pasos, roles o documentos/entidades con nombre propio (ej. tipos de orden, sistemas, etapas de un proceso), nombralos explícitamente en el resumen — no los aplanes a una descripción genérica del resultado final. Alguien que no estuvo en la llamada necesita esos nombres para entender de qué se habló, no solo la conclusión.
...
- "owner": null es SOLO para el acto de coordinar la juntada en sí (fecha/hora). Si dentro de esa reunión de seguimiento hay una acción específica encargada a una persona nombrada (ej. "para que Percy dé su feedback", "Rafa va a evaluar en terreno cómo quedó funcionando"), esa acción es una tarea propia con esa persona como owner — no la fusiones dentro de la tarea genérica de "owner": null de juntarse. Antes de dejar owner en null, revisá si la transcripción realmente no nombra a nadie, o si hay un nombre ahí que corresponde usar.
- Nunca uses el texto "Unknown" como owner. Si no hay un responsable claro, "owner": null.
- No infieras plazos que no se mencionaron explícitamente, pero SÍ capturá condiciones temporales relativas dichas de forma explícita ("antes del jueves", "en menos de 48 horas", "la próxima semana"). El texto de "task" es siempre una instrucción limpia y accionable — nunca repitas ahí la misma duda que ya vas a expresar en "confidence" o en "deadline" (ej. no escribas "(según horario mencionado en la reunión)" en el texto de la tarea si el propio "deadline" ya dice "hora mencionada en la reunión" — la incertidumbre se expresa una sola vez, no dos).
- "confidence": "alta" si el compromiso y el dueño son inequívocos; "media"/"baja" si hay ambigüedad en cualquiera de los dos.
- "possibleOwners": si "confidence" es "alta", siempre array vacío. Si es "media" o "baja", listá ahí cualquier otra persona que pudo ser la responsable real — incluso si igual completaste "owner" con tu mejor candidato — para que quede como señal explícita de "revisar esto", no la dejes vacía solo porque ya elegiste un owner.
...
- Particularidades operativas del estado actual mencionadas al pasar, aunque no sean el tema central de la reunión (ej. "hoy X se registra como si fuera Y", una excepción, un caso borde ya conocido por el equipo). Es común que este tipo de detalle se mencione en una sola frase de contexto antes de pasar a otra cosa — no lo descartes por breve, es exactamente el tipo de particularidad específica del negocio que más vale capturar.

Si la reunión define o repasa un flujo de trabajo con pasos, roles y documentos/entidades con nombre propio (ej. tipos de orden, sistemas, etapas), el ítem de conocimiento tiene que preservar esos nombres tal cual se dijeron — no lo resumas a un nivel tan alto que pierdas qué entidad o etapa específica está involucrada en cada paso.
