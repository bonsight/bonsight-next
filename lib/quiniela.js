...
export const PHASES = {
  ...
  cuartos: {
    ...
    matches: [
      { local: 'G1 Oct.', visitante: 'G2 Oct.', kickoff: '2026-07-09T20:00:00Z', ciudad: 'Foxborough' },
      { local: 'G3 Oct.', visitante: 'G4 Oct.', kickoff: '2026-07-11T21:00:00Z', ciudad: 'Miami' },
      { local: 'G5 Oct.', visitante: 'G6 Oct.', kickoff: '2026-07-10T22:00:00Z', ciudad: 'Los Ángeles' },
      { local: 'G7 Oct.', visitante: 'G8 Oct.', kickoff: '2026-07-12T02:00:00Z', ciudad: 'Kansas City' },
    ],
  },
  semis: {
    label: 'Semifinales',
    matches: [
      { local: 'G1 Cuar.', visitante: 'G2 Cuar.', kickoff: '2026-07-14T19:00:00Z', ciudad: 'Dallas' },
      { local: 'G3 Cuar.', visitante: 'G4 Cuar.', kickoff: '2026-07-15T19:00:00Z', ciudad: 'Atlanta' },
    ],
  },
  final: {
    label: 'Final',
    matches: [{ local: 'G1 Semi', visitante: 'G2 Semi', kickoff: '2026-07-19T19:00:00Z', ciudad: 'Nueva York / NJ' }],
  },
}
...
// Resuelve "G{n} Oct." / "G{n} Cuar." / "G{n} Semi" al equipo real usando resultados confirmados.
// Útil para mostrar nombres reales en cuartos/semis/final sin cambiar cómo se guardan los picks.
export function resolveKnockoutName(placeholder, adminResults) {
  if (!placeholder || !adminResults) return placeholder
  function winnerOf(phase, idx) {
    const result = adminResults?.[phase]?.[idx]
    const match = PHASES[phase]?.matches?.[idx]
    if (!match || !isMatchFinal(result)) return null
    const local = resolveKnockoutName(match.local, adminResults) ?? match.local
    const visitante = resolveKnockoutName(match.visitante, adminResults) ?? match.visitante
    if (result.ganador === 'local') return local
    if (result.ganador === 'visitante') return visitante
    const l = Number(result.l), v = Number(result.v)
    if (l > v) return local
    if (l < v) return visitante
    return null
  }
  const oct  = placeholder.match(/^G(\d+) Oct\.$/)
  if (oct)  return winnerOf('octavos', parseInt(oct[1]) - 1)  ?? placeholder
  const cuar = placeholder.match(/^G(\d+) Cuar\.$/)
  if (cuar) return winnerOf('cuartos', parseInt(cuar[1]) - 1) ?? placeholder
  const semi = placeholder.match(/^G(\d+) Semi$/)
  if (semi) return winnerOf('semis',   parseInt(semi[1]) - 1)  ?? placeholder
  return placeholder
}
...
// real.ganador = 'local' | 'visitante' cuando el partido se definió en ET/penales (knockout)
export function evaluatePick(pick, real, match) {
  ...
  const realWin =
    real.ganador === 'local'    ? match.local
    : real.ganador === 'visitante' ? match.visitante
...
export function calcularPuntajes(participantes, quinielas, admin, { live = false } = {}) {
  return participantes.map((p) => {
    ...
    const byPhase = Object.fromEntries(PHASE_ORDER.map(ph => [ph, 0]))
    let pts = 0

    if (!q) return { participantId: p.id, pts, breakdown, byPhase }
    ...
    return { participantId: p.id, pts, breakdown, byPhase }
  })
}
