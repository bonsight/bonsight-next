...
export function pageToFields(page) {
  ...
  for (const [key, val] of Object.entries(props)) {
    ...
    // formula: number/string/boolean van directo, date viene anidado como { start, end,
    // is_datetime } igual que una propiedad date normal — por eso el caso aparte.
    else if (val.type === 'formula') {
      fields[key] = val.formula?.type === 'date' ? (val.formula.date?.start ?? null) : (val.formula?.[val.formula?.type] ?? null);
    }
  }
