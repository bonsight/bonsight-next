...
export async function testAndIntrospect(source) {
  ...
  if (type === 'bigquery') return introspectBigQuery(connectionString);
...
function parseBigQueryCredentials(connectionString) {
  try {
    const creds = JSON.parse(connectionString);
    if (!creds.project_id || !creds.private_key) throw new Error('JSON inválido: falta project_id o private_key.');
    return creds;
  } catch (e) {
    throw new Error(`Credenciales BigQuery inválidas: ${e.message}`);
  }
}

async function introspectBigQuery(connectionString) {
  const creds = parseBigQueryCredentials(connectionString);
  const { BigQuery } = await import('@google-cloud/bigquery');
  const bq = new BigQuery({ projectId: creds.project_id, credentials: creds });

  const [datasets] = await bq.getDatasets();
  const tables = [];
  const datasetSlice = datasets.slice(0, 20);

  for (const dataset of datasetSlice) {
    const [tbls] = await dataset.getTables();
    for (const tbl of tbls.slice(0, 30)) {
      try {
        const [meta] = await tbl.getMetadata();
        const fields = (meta.schema?.fields ?? []).map((f) => ({ column: f.name, type: f.type }));
        tables.push({ name: `${dataset.id}.${tbl.id}`, columns: fields });
...
async function queryBigQuery(connectionString, sql) {
  const creds = parseBigQueryCredentials(connectionString);
  const { BigQuery } = await import('@google-cloud/bigquery');
  const bq = new BigQuery({ projectId: creds.project_id, credentials: creds });

  const [rows] = await bq.query({ query: sql, maximumBytesBilled: '1000000000', timeoutMs: QUERY_TIMEOUT_MS });
  const limited = rows.slice(0, ROW_LIMIT);
  const fields = limited.length ? Object.keys(limited[0]) : [];
...
export async function queryDatabase(source, sql) {
  ...
  if (type === 'bigquery') return queryBigQuery(connectionString, sql);
...
export function buildDbSourcesContext(sources) {
  ...
  return active.map((s) => {
    ...
    const note = s.type === 'bigquery' ? ' (BigQuery SQL — referencias: `dataset.tabla`)' : '';
