...
export async function updateTaskDetails(token, pageId, { title, description }) {
  const properties = {};
  if (title !== undefined) {
...
// Inicio/Finalizado tienen que caer dentro del rango del sprint al que pertenece la tarea —
// bloqueo duro, no solo aviso (decisión del usuario). El date input del cliente ya lo acota
// con min/max, esto es el resguardo del lado del servidor.
function validateWithinSprint(sprint, startDate, endDate) {
  ...
  if (sprint.startDate && sprint.endDate) {
    if (startDate && (startDate < sprint.startDate || startDate > sprint.endDate)) {
      throw new Error(`El inicio tiene que estar dentro del sprint (${sprint.startDate} – ${sprint.endDate}).`);
    }
...
export async function createTask(token, sprintId, { title, status, proyectoId, responsableId, priority, taskType, severity, iniciativaId, parentId, startDate, endDate, estimatedHours, description }) {
  ...
  if (startDate) properties['Inicio'] = { date: { start: startDate } };
