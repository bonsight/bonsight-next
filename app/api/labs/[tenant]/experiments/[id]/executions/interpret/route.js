...
export async function POST(req, { params }) {
  ...
  const { testId, freeText } = await req.json();
  if (!testId || !freeText?.trim()) {
    return Response.json({ error: 'testId y freeText son requeridos.' }, { status: 400 });
  }
  ...
  try {
    const interpreted = await interpretContribution(test, freeText);
