...
export async function POST(req, { params }) {
  ...
  try {
    transcription = await openai.audio.transcriptions.create({
      ...
      model: 'whisper-1',
      language: 'es',
      prompt: 'Análisis de negocios en español, métricas, KPIs, conversión, tráfico web, estrategia.',
    });
  } catch (err) {
    console.error(`[aria/${tenant}/transcribe] OpenAI error:`, err?.message || err);
